...
```{r}
m2
coef(m2) # no slope = slope of zero
...
uniforms_long <- lm(Size ~ Height + Height:Weight, data = Student_stats)

uniforms_2 <- lm(Size ~ Height + Height:Weight, data = Student_stats)

AIC(uniforms_long, uniforms_2)
```
...
```{r}
test <- lm(mpg ~ wt + qsec + am + hp +wt:hp, data = mtcars)
summary(test)
