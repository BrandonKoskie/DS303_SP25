...
The data is showing whether the client is a student or not and the balance and income. Missing and complete rate is binary.
...
```{r}
default_model1 <- glm(default ~ income, data = default, family = binomial)
summary(default_model)
```

Interpret the results below:

For each \$1 increase in income the log-odds of defaulting decrease by 0.000008353. The coefficient is negative which suggests a higher income is associated with a lower chance of defaulting.
...
```{r}
default_model2 <- glm(default ~ income + balance + student, data = default, family = binomial)
summary(default_model2)

AIC(default_model, default_model2)

```

Interpret your findings here:

default_model2 is the better choice
...
mtcars$am <- factor(mtcars$am, labels = c("Automatic", "Manual"))  # Convert to categorical

# build model
transmission_model <- glm(am ~ mpg + hp + wt, data = mtcars, family = binomial)
