...
-   **(Intercept)**: -43.781
    ...
    -   This is the log-odds of versacolor when the petal length is zero.
    -   Value is not meaningful as petal lengths are positive
...
```{r}
flower <- glm(Species ~ Sepal.Length, data = iris_subset, family = binomial)
summary(flower)
```
...
-   **(Intercept)**: -12.5708
    ...
    -   This is the log-odds of a flower being verginica.

-   **your predictor**: Sepal.Length

    -   This coefficient represents the increase in log odds for being verginica for each 1 unit increase in Sepal.Length.
